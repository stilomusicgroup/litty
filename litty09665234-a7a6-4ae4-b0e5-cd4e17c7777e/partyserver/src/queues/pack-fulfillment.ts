...
function isFrozen(env: Record<string, unknown>): boolean {
  return env.FULFILLMENT_FROZEN === 'true' || process.env.FULFILLMENT_FROZEN === 'true';
}

export const packFulfillment: QueueHandler<PackFulfillmentPayload> = async (job, context) => {
  ...
  if (isFrozen(context.env)) {
    if ((context.env as any).LOG_LEVEL !== 'silent') console.log(`[pack-fulfillment] FROZEN — skipping job ${job.jobId} for purchase ${purchaseId} (vault drain freeze 2026-05-19)`);
    await context.updateStatus({ message: `FROZEN: fulfillment halted (vault drain freeze 2026-05-19)`, metadata: { frozen: true } });
    return;
  }

  if ((context.env as any).LOG_LEVEL !== 'silent') console.log(`[pack-fulfillment] Starting job ${job.jobId} for purchase ${purchaseId} (attempt ${job.attempts})`);
  ...
  if (existing) {
    ...
    if (status === 'completed' || status === 'needs_review') {
      if ((context.env as any).LOG_LEVEL !== 'silent') console.log(`[pack-fulfillment] Purchase ${purchaseId} already terminal (status=${status}) — skipping`);
  ...
  if ((context.env as any).LOG_LEVEL !== 'silent') console.log(
    `[pack-fulfillment] runAllSteps complete for ${purchaseId}: ` +
  ...
  if (!stepsResult.stepTokens) {
    throw new Error(
      `[pack-fulfillment] stepTokens failed for ${purchaseId} (status=${finalStatus}) — will retry`,
    );
  }
    ...
    if (shopifyEnv.SHOPIFY_ADMIN_API_TOKEN && shopifyEnv.SHOPIFY_STORE_DOMAIN_V2) {
      try {
        ...
        if (fulfillResult.fulfilled) {
          if ((context.env as any).LOG_LEVEL !== 'silent') console.log(`[pack-fulfillment] Shopify order ${orderId} marked fulfilled (fulfillmentId=${fulfillResult.fulfillmentId})`);
  ...
  if ((context.env as any).LOG_LEVEL !== 'silent') console.log(`[pack-fulfillment] Job ${job.jobId} complete for ${purchaseId}: ${finalStatus}`);
};
 ...
 */
async function sendQueueReceiptEmail(params: { purchaseId: string; email: string }): Promise<void> {
  ...
  const songId = (finalRecord as any).songId ?? '';
  const [song, songDetails] = await Promise.all([
    songId ? getSongs(songId) : null,
    songId ? getSongDetails(songId) : null,
  ]);

  const songTitle = (songDetails as any)?.title ?? song?.name ?? 'Your Song';
  const artistName = (songDetails as any)?.artist ?? 'Unknown Artist';
  const tokenAmount = typeof (finalRecord as any).tokenAmount === 'number' ? (finalRecord as any).tokenAmount : undefined;
  const tokenSymbol = song?.symbol ?? undefined;
  const splTxHash = (finalRecord as any).splTxHash ?? undefined;

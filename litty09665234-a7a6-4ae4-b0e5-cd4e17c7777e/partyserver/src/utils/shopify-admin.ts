...
function getAdminBaseUrl(env: ShopifyAdminEnv): string {
...
function getHeaders(env: ShopifyAdminEnv): Record<string, string> {
...
// ── Sentinel result type for Admin API order verification ─────────────────────
 ...
 */
export async function verifyOrderViaAdminApi(
  env: ShopifyAdminEnv,
  orderId: string,
  ...
  try {
    ...
    console.warn(
      `[ShopifyAdmin] Network error fetching order ${orderId}: ${networkErr instanceof Error ? networkErr.message : String(networkErr)}`,
    );
  ...
  if (res.status === 401 || res.status === 403) {
    ...
    console.warn(
      `[ShopifyAdmin] Admin API returned ${res.status} for order ${orderId}. ` +
  ...
  if (!res.ok) {
    console.warn(
      `[ShopifyAdmin] Admin API returned unexpected status ${res.status} for order ${orderId} — falling through to HMAC-only`,
    );
    return { verified: false, order: null, reason: 'admin_unavailable' };
  }
...
async function parseShopifyError(res: Response): Promise<string> {
  if (res.status === 401) return 'invalid admin token (401 Unauthorized)';
  if (res.status === 403) return 'insufficient permissions (403 Forbidden)';
  if (res.status === 404) return 'not found (404)';
  if (res.status === 429) return 'rate limited (429 Too Many Requests)';
  try {
...
export async function getFulfillmentOrders(
  ...
  const json = await res.json() as { fulfillment_orders?: ShopifyFulfillmentOrder[] };
...
export async function createFulfillment(
  ...
  const json = await res.json() as { fulfillment?: ShopifyFulfillmentResult };
  if (!json.fulfillment) {
...
export async function fulfillShopifyOrder(
  env: ShopifyAdminEnv,
  orderId: string,
): Promise<{ fulfilled: boolean; fulfillmentId?: string; reason?: string }> {
  let fulfillmentOrders: ShopifyFulfillmentOrder[];
  try {
  ...
  let lastFulfillmentId: string | undefined;

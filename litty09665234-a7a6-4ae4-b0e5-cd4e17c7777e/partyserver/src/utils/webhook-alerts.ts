...
import { Time } from '../db-client.js';
import {
  setWebhookFailures,
  updateWebhookFailures,
  getManyWebhookFailures,
} from '../collections/webhookFailures.js';
...
export async function logWebhookFailure(params: {
  ...
  try {
    ...
    const recentFailures = await getManyWebhookFailures(
      `where alertSent = false and timestamp > ${tenMinAgo} order by timestamp desc limit 50`,
    );
    ...
    if (recentFailures.length >= ALERT_THRESHOLD) {
      ...
      if (discordRes.ok) {
        ...
        const updatePromises = recentFailures.map(f =>
          updateWebhookFailures(f.id, { alertSent: true }).catch(updateErr =>
            console.warn(`[WebhookAlert] Failed to mark failure ${f.id} as alerted:`, updateErr),
          ),
        );

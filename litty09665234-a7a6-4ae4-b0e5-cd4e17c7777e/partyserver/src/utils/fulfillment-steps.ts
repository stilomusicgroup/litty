...
import { getSongDetails, updateSongDetails, runTokenPriceUsdQueryForSongDetails } from '../collections/songDetails.js';
import {
  getPackPurchases,
  updatePackPurchases,
  setPackPurchasesPayouts,
  setPackPurchasesInfraFees,
  setPackPurchasesTreasuryFees,
...
import { setFile, getFiles, Time, Address, Increment } from '../db-client.js';
import { ensureVaultHasSongTokens } from './bonding-curve-buy.js';
import { PROJECT_VAULT_ADDRESS, ADMIN_ADDRESS, OPERATIONS_WALLET, TREASURY_WALLET as TREASURY_WALLET_CONST } from '../constants.js';
import {
  captureSolPriceUsd,
  scaleToMicroUsd,
  unscaleFromMicroUsd,
} from './price-capture.js';
...
export const SHOPIFY_SHARES    = { airdrop: 0.90, treasury: 0.02, infra: 0.03,  funding: 0.00 } as const;
export const DIRECT_SOL_SHARES = { airdrop: 0.92, treasury: 0.015, infra: 0.015, funding: 0.00 } as const;
...
if (Math.abs(_shopifySum - 1.0) > 1e-9) throw new Error(`FATAL: SHOPIFY_SHARES sum is ${_shopifySum}, must be 1.0`);
if (Math.abs(_directSolSum - 1.0) > 1e-9) throw new Error(`FATAL: DIRECT_SOL_SHARES sum is ${_directSolSum}, must be 1.0`);
...
async function getVaultTokenBalance(songIdStr: string): Promise<number> {
  // Fetch the song record to see if mintAddress is stored
  const song = await getSongs(songIdStr);
  const storedMint: string | undefined = (song as any)?.mintAddress;
...
async function writeFailedFulfillment(params: {
  ...
  try {
    ...
    const solAmt: number | undefined = solPriceAtPurchaseUsd
      ? Math.floor(usdAmt / (solPriceAtPurchaseUsd / 1_000_000) * 1_000_000_000)
      : undefined;
...
function buildNftMetadata(params: {
  ...
  if (hasAudio) {
    ...
    (metadata.properties as any).files.push({ uri: `${audioUrl}?ext=mp3`, type: 'audio/mpeg' });
  }
 ...
 */
export async function runStepCapturePrice(purchaseId: string): Promise<number | null> {
  ...
  if (captured === null) {
    await updatePackPurchases(purchaseId, { status: 'pending_price_fetch' } as any);
    await logFailure({
      purchaseId,
...
// ─── Helper: derive lamport budgets from purchase record ──────────────────────
...
interface FulfillmentContext {
  ...
  packIdVal: string;
  isDirectSol: boolean;
  shares: typeof SHOPIFY_SHARES | typeof DIRECT_SOL_SHARES;
  purchaseAmountUsd: number;
  solPriceUsd: number;
  /** lamports for fan airdrop (bonding curve buy budget) */
  airdropLamports: number;
  /** lamports for outbound treasury transfer */
  treasuryLamports: number;
  /** lamports equivalent for infra (vault-retained; for accounting only) */
  infraLamports: number;
  /** lamports equivalent for funding (vault-retained; for accounting only) */
  fundingLamports: number;
  /** lamports for artist token buy (runStepPayout) */
  artistTokenBudgetLamports: number;
}

async function buildFulfillmentContext(purchaseId: string): Promise<FulfillmentContext | null> {
  const purchase = await getPackPurchases(purchaseId);
  if (!purchase) {
    return null;
  }

  const { songId, buyerAddress, walletSource } = purchase as any;
  const songIdStr = songId as string | undefined;
  ...
  const rawPurchaseAmountUsd = (purchase as any).purchaseAmountUsd;
  if (rawPurchaseAmountUsd == null || rawPurchaseAmountUsd <= 0) {
    ...
    await writeFailedFulfillment({
      ...
      errorMessage: errMsg,
      songIdVal: songIdStr,
      buyerAddr: buyerAddress as string,
    });
    return null;
  }
  const purchaseAmountUsd: number = rawPurchaseAmountUsd;

  const shares = isDirectSol ? DIRECT_SOL_SHARES : SHOPIFY_SHARES;
  const lamports = (pct: number) => Math.floor((purchaseAmountUsd * pct) / solPriceUsd * 1_000_000_000);
  ...
  return {
    ...
    packIdVal: (purchase as any).packId ?? '',
    isDirectSol,
    shares,
    purchaseAmountUsd,
    solPriceUsd,
 ...
 */
export async function runStepAirdrop(purchaseId: string): Promise<boolean> {
  const purchase = await getPackPurchases(purchaseId);
  if (!purchase) {
    return false;
  }
  ...
  // Idempotency
  if ((purchase as any).airdropCompleted === true) {
    return true;
  }
  ...
  if ((purchase as any).swapFailedAt) {
    const errMsg = `Swap previously failed at ${(purchase as any).swapFailedAt} for ${purchaseId} — non-retriable. Admin must clear swapFailedAt before retry.`;
  ...
  {
    const { songId: airdropSongId } = purchase as any;
    if (airdropSongId) {
      const songForCheck = await getSongs(airdropSongId as string);
      const storedMintCheck: string | undefined = (songForCheck as any)?.mintAddress;
      if (!storedMintCheck || storedMintCheck.trim() === '') {
        ...
        await updatePackPurchases(purchaseId, { status: 'partial_failure' } as any);
        await logFailure({ purchaseId, step: 'stepAirdrop', errorMessage: errMsg, songId: airdropSongId as string });
        await writeFailedFulfillment({
          purchaseId,
          failureStage: 'airdrop',
          errorMessage: errMsg,
          songIdVal: airdropSongId as string,
        });
        return false;
      }
    }
  ...
  if ((purchase as any).bondingCurveBuySubmitted === true) {
    const existingBuyId = (purchase as any).bondingCurveBuyId as string | undefined;
    ...
    const storedTokenAmount = (purchase as any).tokenAmount ?? 0;
    let actualFanTokens2: number;
    if (storedTokenAmount > 0) {
      ...
      if (vaultBalance2 <= 0) {
        ...
        await updatePackPurchases(purchaseId, { status: 'needs_review', needsReviewReason: errMsg2 } as any);
      ...
      const expectedFanTokens2 = (purchase as any).tokenAmount ?? 0;
    ...
    if ((purchaseBeforeAirdrop2 as any)?.airdropCompleted === true) {
      return true;
    }
    ...
    if (!airdropOk2) {
      await updatePackPurchases(purchaseId, { status: 'partial_failure', splTxHash: splTxHash2 } as any);
      await writeFailedFulfillment({
        purchaseId,
        failureStage: 'airdrop',
  ...
  if (seedOk && bondingCurveBuyId) {
    const tokenAmountToWrite = capturedTokenAmount > 0 ? capturedTokenAmount : ((purchase as any).tokenAmount ?? 0);
    try {
      await updatePackPurchases(purchaseId, {
  ...
  if (!seedOk) {
    await updatePackPurchases(purchaseId, { status: 'partial_failure', swapFailedAt: Time.Now } as any);
    await writeFailedFulfillment({
      purchaseId,
      failureStage: 'airdrop',
  ...
  if (capturedTokenAmount > 0) {
    ...
    const VAULT_BALANCE_CHECK_DELAY_MS = 2_000;
    let vaultBalance = 0;
    ...
    // balance and may include leftovers from a prior failed airdrop or concurrent
    // purchases sharing the same vault (OPERATIONS_WALLET === PROJECT_VAULT_ADDRESS).
    const expectedFanTokens = (purchase as any).tokenAmount ?? 0;
  ...
  if ((purchaseBeforeAirdrop as any)?.airdropCompleted === true) {
    return true;
  }
  ...
  if (!airdropOk) {
    await updatePackPurchases(purchaseId, { status: 'partial_failure', splTxHash } as any);
    await writeFailedFulfillment({
      purchaseId,
      failureStage: 'airdrop',
 ...
 */
export async function runStepTreasuryTransfer(purchaseId: string): Promise<boolean> {
  const purchase = await getPackPurchases(purchaseId);
  if (!purchase) {
    return false;
  }
  ...
  // Idempotency
  if ((purchase as any).treasuryTransferCompleted === true) {
    return true;
  }
 ...
 */
export async function runStepInfraFee(purchaseId: string): Promise<boolean> {
  const purchase = await getPackPurchases(purchaseId);
  if (!purchase) {
    return false;
  }
  ...
  // Idempotency
  if ((purchase as any).infraFeeAccounted === true) {
    return true;
  }
 ...
 */
export async function runStepFundingFee(purchaseId: string): Promise<boolean> {
  const purchase = await getPackPurchases(purchaseId);
  if (!purchase) {
    return false;
  }
  ...
  // Idempotency
  if ((purchase as any).fundingFeeAccounted === true) {
    return true;
  }
 ...
 */
export async function runStepTokens(purchaseId: string, _env?: Record<string, string>): Promise<boolean> {
  const purchase = await getPackPurchases(purchaseId);
  if (!purchase) {
    return false;
  }
  ...
  await updatePackPurchases(purchaseId, { stepTokens: true } as any);
 ...
 */
export async function runStepPayout(purchaseId: string): Promise<boolean> {
  const purchase = await getPackPurchases(purchaseId);
  if (!purchase) { return false; }
  if (purchase.stepPayout === true) { return true; }
  ...
  if ((purchase as any).artistPayoutSubmitted === true) {
    await updatePackPurchases(purchaseId, {
      stepPayout: true,
  ...
  if (!artistAddress || artistAddress === PROJECT_VAULT_ADDRESS) {
    await updatePackPurchases(purchaseId, { stepPayout: true, artistPayoutStatus: 'skipped' } as any);
    return true;
  }
  ...
  const rawPayoutAmountUsd = (purchase as any).purchaseAmountUsd;
  ...
  const purchaseAmountUsd: number = rawPayoutAmountUsd;

  const isDirectSolPayout = (purchase as any).walletSource === 'direct_sol';
  const shares = isDirectSolPayout ? DIRECT_SOL_SHARES : SHOPIFY_SHARES;

  const artistTokenBudgetLamports = Math.floor((purchaseAmountUsd * ARTIST_TOKEN_SHARE) / solPriceUsd * 1_000_000_000);
  ...
  if (artistTokenBudgetLamports > 0) {
    try {
      ...
      if (seedResult.ok) {
        ...
        try {
          await updatePackPurchases(purchaseId, { artistPayoutSubmitted: true } as any);
        } catch (flagErr) {
        }
  ...
  const tipPercent = Number((purchase as any).tipPercent ?? 0);
  ...
  if (hasTip) {
    ...
    artistPayoutSOL = Math.floor((artistPayoutUSD / solPriceUsd) * 1_000_000_000);
 ...
 */
export async function runStepNft(purchaseId: string): Promise<boolean> {
  const purchase = await getPackPurchases(purchaseId);
  if (!purchase) { return false; }
  if (purchase.stepNft === true) { return true; }

  // ── NFT MINTING DISABLED ──────────────────────────────────────────────────
  await updatePackPurchases(purchaseId, { stepNft: true, nftFailedCount: 0 } as any);
  return true;
 ...
 */
export async function runStepNotify(purchaseId: string): Promise<boolean> {
  const purchase = await getPackPurchases(purchaseId);
  if (!purchase) { return false; }
  if (purchase.stepNotify === true) { return true; }

  const { songId, buyerEmail, buyerAddress, packName, nftCount, tokenAmount } = purchase as any;
  const songIdStr = songId as string | undefined;

  if (!buyerEmail) {
    await updatePackPurchases(purchaseId, { stepNotify: true } as any);
    return true;
  }
  ...
  try {
    const song = await getSongs(songIdStr ?? '');
    if (song) songName = song.name ?? 'Unknown Song';
  } catch (songErr) {
  }
 ...
 */
export async function runAllSteps(purchaseId: string): Promise<StepsResult> {
  const purchase = await getPackPurchases(purchaseId);
  if (!purchase) {
    return {
  ...
  const airdropAlreadyDone = (freshPurchase as any)?.airdropCompleted === true;
  const treasuryAlreadyDone = (freshPurchase as any)?.treasuryTransferCompleted === true;
  const infraAlreadyDone = (freshPurchase as any)?.infraFeeAccounted === true;
  const fundingAlreadyDone = (freshPurchase as any)?.fundingFeeAccounted === true;
  ...
  await updatePackPurchases(purchaseId, { status: finalStatus } as any);

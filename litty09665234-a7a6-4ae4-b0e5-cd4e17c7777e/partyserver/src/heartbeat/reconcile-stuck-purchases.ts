...
const KNOWN_MISSING_SONGS: Record<string, {
  name: string;
  symbol: string;
  uri: string;
  creator: string;
...
export async function reconcileStuckPurchases(): Promise<void> {
  if (isFrozen()) {
  ...
  try {
    ...
    const stuckPurchases = (allPurchases as any[]).filter((p: any) => {
      if (!STUCK_STATUSES.has(p.status)) return false;
      ...
      if (p.status === 'pending_insufficient_funds') return true;
      const createdAt = (p as any).createdAt ?? 0;
    ...
    const recentPurchases = stuckPurchases.filter((p: any) => {
      const createdAt = (p as any).createdAt ?? 0;
    ...
    for (const purchase of recentPurchases) {
      ...
      const songId = (purchase as any).songId;
      const status = (purchase as any).status;
      const packId = (purchase as any).packId;
      const shopifyOrderId = (purchase as any).shopifyOrderId;
      ...
      if ((purchase as any).swapFailedAt) {
        const reason = `Swap previously failed at ${(purchase as any).swapFailedAt} — non-retriable, marking needs_review`;
        console.warn(`[heartbeat] ${purchaseId}: ${reason}`);
        await updatePackPurchases(purchaseId, { status: 'needs_review', needsReviewReason: reason } as any);
        stillFailed++;
        continue;
      }
      ...
      if (!song) {
        ...
        try {
          await updatePackPurchases(purchaseId, { status: 'failed' } as any);
        } catch (err) {
      ...
      try {
        ...
        const reconcileAttempts = (purchase as any).reconcileAttempts as number ?? 0;
        if (reconcileAttempts >= 3) {
          // Terminal: exceeded auto-retry cap. Flip to needs_review so admins can triage manually.
          const lastError = (purchase as any).lastErrorMessage as string | undefined;
          const lastStep = (purchase as any).lastStepName as string | undefined;
          ...
          console.warn(
            `[heartbeat] ${purchaseId} has hit max reconcile retries (${reconcileAttempts}) — marking needs_review: ${needsReviewReason}`,
          );
          await updatePackPurchases(purchaseId, { status: 'needs_review', needsReviewReason } as any);
          stillFailed++;
          continue;
        }
        // Increment attempt counter
        await updatePackPurchases(purchaseId, { reconcileAttempts: reconcileAttempts + 1 } as any);
        ...
        let stepResult: Awaited<ReturnType<typeof runAllSteps>> | null = null;
        try {
        ...
        if (isRecovered) {
          ...
          const alreadyFulfilled = (freshPurchase as any)?.shopifyFulfilled === true;
          if (alreadyFulfilled) {
    ...
    console.log(
      `[heartbeat] reconcile-stuck-purchases: scan complete — ` +

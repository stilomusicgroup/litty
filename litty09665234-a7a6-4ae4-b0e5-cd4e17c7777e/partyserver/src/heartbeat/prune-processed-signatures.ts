...
export async function pruneProcessedSignatures(): Promise<void> {
  ...
  console.log(
    `[prune-processed-signatures] ${entries.length} total entries. Cutoff: ${cutoffSeconds} (${new Date(cutoffSeconds * 1000).toISOString()})`
  );
  ...
  console.log(
    `[prune-processed-signatures] Done. Pruned ${succeeded}/${stale.length} stale entries` +
  ...
  if (failed.length > 0 && succeeded === 0) {
    throw new Error(
      `[prune-processed-signatures] All ${failed.length} deletes failed. Check logs above.`
    );
  }
}

...
export async function backfillSongCreators(): Promise<void> {
  console.log('[backfill-song-creators] Starting...');

  // ── 1. Fetch all collections ───────────────────────────────────────────────

  const allSongs = (await get('songs')) as any[] | null;
  if (!allSongs || !Array.isArray(allSongs)) {
    console.error('[backfill-song-creators] Failed to fetch songs collection or it is empty');
    return;
  }

  const allSongDetails = (await get('songDetails')) as any[] | null;
  const allUsers = (await get('users')) as any[] | null;

  // ── 2. Build lookup maps ───────────────────────────────────────────────────

  // songId → artistEmail
  const songIdToEmail: Record<string, string> = {};
  if (Array.isArray(allSongDetails)) {
    for (const sd of allSongDetails) {
      const id = sd.id as string | undefined;
      const email = sd.artistEmail as string | undefined;
  ...
  const emailToWallet: Record<string, string> = {};
  if (Array.isArray(allUsers)) {
    for (const u of allUsers) {
      const email = u.email as string | undefined;
      const wallet = u.walletAddress as string | undefined;
  ...
  const updates: { songId: string; wallet: string }[] = [];
  const unresolved: { songId: string; reason: string }[] = [];

  for (const song of allSongs) {
    totalScanned++;
    const songId = song.id as string;
    const creator = song.creator as string | undefined | null;
    ...
    if (wallet === undefined) {
      ...
      const matchingUser = Array.isArray(allUsers)
        ? allUsers.find((u: any) => (u.email as string | undefined)?.trim().toLowerCase() === email)
        : undefined;
  ...
  for (const u of unresolved) {
    if (sampleUnresolved.length >= 5) break;

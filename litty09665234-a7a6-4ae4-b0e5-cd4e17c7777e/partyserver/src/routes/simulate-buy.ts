...
const MIN_BALANCE_BUFFER_LAMPORTS = 500_000; // 0.0005 SOL buffer
const ATA_RENT_LAMPORTS = 2_039_280; // rent-exempt balance for associated token account
const TX_FEE_LAMPORTS = 15_000; // generous tx fee estimate

async function fetchPumpFunCoin(mintAddress: string): Promise<PumpFunCoin | null> {
  try {
...
export function registerSimulateBuyRoute(app: Hono) {
  app.post('/api/songs/:songId/simulate-buy', async (c) => {
    ...
    let body: { solAmt?: number; slipBps?: number; walletAddress?: string };
    try {
    ...
    const platformFeeLamports = Math.floor((solAmt * Number(PLATFORM_FEE_BPS)) / 10_000);
    ...
    if (buyerBalanceLamports < estimatedTotalLamports) {
      const deficit = ((estimatedTotalLamports - buyerBalanceLamports) / 1_000_000_000).toFixed(6);
      errors.push(
        `Insufficient SOL balance. Need ~${(estimatedTotalLamports / 1_000_000_000).toFixed(6)} SOL ` +
        `(includes ${(solAmt / 1_000_000_000).toFixed(4)} buy + platform fee + tx fee + possible ATA rent). ` +
    ...
    try {
      ...
      } else if (simResult.value.unitsConsumed && simResult.value.unitsConsumed > 1_300_000) {

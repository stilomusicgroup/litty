...
export function registerArtistWalletRoutes(app: Hono): void {
  app.post('/api/artists/ensure-wallet', async (c) => {
    ...
    const email = (artist as any).email as string | undefined;
    if (!email || email.trim() === '') {
      return ApiErrors.badRequest(
        c,
        'Artist profile is missing an email address. Please update your profile with a valid email first.',
      );
    }

    const PRIVY_APP_SECRET = (c.env as any).PRIVY_APP_SECRET as string | undefined;
    if (!PRIVY_APP_SECRET) {
    ...
    try {
      const getUserRes = await fetch(
        `https://api.privy.io/v1/users/email:${encodeURIComponent(email.trim())}`,
        { headers: privyHeaders },
      );

      if (getUserRes.ok) {
        const userData = (await getUserRes.json()) as {
        ...
        if (linkedWallet?.address) {
          if ((c.env as any).LOG_LEVEL !== 'silent') console.log(`[ArtistWallet] Found existing Privy wallet for ${email}: ${linkedWallet.address}`);
    ...
    if (!privyWalletAddress) {
      try {
        ...
        const walletData = (await createWalletRes.json()) as { address: string };
        ...
        if ((c.env as any).LOG_LEVEL !== 'silent') console.log(`[ArtistWallet] Created Privy embedded wallet for artist ${walletAddress} / ${email}: ${privyWalletAddress}`);
      } catch (err) {

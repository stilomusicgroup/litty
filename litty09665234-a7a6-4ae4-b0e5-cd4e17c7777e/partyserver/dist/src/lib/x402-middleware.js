...
export const x402Middleware = Object.keys(paidRoutes).length > 0
    ? async (c, next) => {
        if (!_x402Inner) {
            _x402Inner = paymentMiddlewareFromConfig(paidRoutes, undefined, [{ network, server: new ExactSvmScheme() }]);
        }
        try {
            ...
            return c.json({ success: false, error: 'Payment processing temporarily unavailable' }, 500);
        }
    }
    : (_c, next) => next();

...
async function fanoutTo(req, routePrefix, sentinelBase, sentinelRoot) {
    ...
    for (const [k, v] of req.headers) {
        ...
        if (STRIPPED_HEADERS.has(key))
            continue;
        if (STRIPPED_PREFIXES.some((p) => key.startsWith(p)))
            continue;
        headers.set(k, v);
    }
 ...
 */
function checkFanoutToken(req) {
    const expected = process.env.POOF_FANOUT_TOKEN;
    if (!expected) {
...
async function gatedFanout(req, routePrefix, sentinelBase, sentinelRoot) {
...
export function registerMcpFanoutRoutes(app) {

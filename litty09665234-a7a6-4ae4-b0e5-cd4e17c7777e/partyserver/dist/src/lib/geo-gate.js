...
function getRestrictedCountries(env, restriction) {
    const raw = env && typeof env === 'object'
        ? env[restriction.envVar]
        : undefined;
...
export function isProgramBlocked(c, program) {
...
export function assertProgramAllowed(c, program) {
    const restriction = RESTRICTIONS[program];
    if (!restriction)
        return;
    if (!isProgramBlocked(c, program))
        return;
    const country = getViewerCountry(c);

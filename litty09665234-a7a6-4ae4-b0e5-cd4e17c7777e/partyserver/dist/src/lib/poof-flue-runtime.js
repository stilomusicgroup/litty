...
import { bashFactoryToSessionEnv, configureFlueRuntime, createDurableRunStore, createFlueContext, createRunSubscriberRegistry, handleAgentRequest, handleRunRouteRequest, resolveModel, } from '@flue/sdk/internal';
...
function resolvePoofFanoutBase() {
    const explicit = process.env.POOF_FANOUT_BASE_URL;
    if (explicit)
        return explicit;
    const appId = process.env.TAROBASE_APP_ID;
    if (!appId) {
        throw new Error('[poof-flue] Cannot resolve Poof fanout base URL: neither POOF_FANOUT_BASE_URL nor TAROBASE_APP_ID is set.');
    }
    const envName = process.env.ENV;
    const suffix = envName === 'LIVE' ? 'poof.new' : 'proxy-staging.poof.new';
    return `https://${appId}-api.${suffix}/__poof`;
}
function resolveMcpFanoutUrl() {
    return `${resolvePoofFanoutBase()}/mcp`;
}
function resolveAiFanoutBase() {
    return `${resolvePoofFanoutBase()}/ai`;
}
function resolveWalletFanoutUrl() {
    return `${resolvePoofFanoutBase()}/wallet`;
}
function isWalletEnabled() {
    return process.env.POOF_WALLET_ENABLED === 'true';
}
function resolveWalletScopeId() {
    ...
    return process.env.POOF_WALLET_SCOPE_ID || 'default';
}
...
function resolveFanoutToken() {
    return process.env.POOF_FANOUT_TOKEN;
}
...
function createOnchainosShim(fanoutUrl, fanoutToken, walletScopeId) {
    return {
        name: 'onchainos',
        execute: async (args) => {
            try {
                ...
                if (!res.ok) {
                    ...
                    console.warn('[poof-wallet] fanout returned non-ok:', JSON.stringify({ status: res.status, subcommand: args[0] }));
                    return {
                        stdout: '',
                ...
                console.warn('[poof-wallet] shim fetch threw:', err instanceof Error ? err.message : String(err));
                return {
                    stdout: '',
...
function createDOStore(sql) {
    sql.exec('CREATE TABLE IF NOT EXISTS flue_sessions (id TEXT PRIMARY KEY, data TEXT NOT NULL, updated_at INTEGER NOT NULL)');
    return {
        async save(id, data) {
            const json = JSON.stringify(data);
            sql.exec('INSERT OR REPLACE INTO flue_sessions (id, data, updated_at) VALUES (?, ?, ?)', id, json, Date.now());
        },
        async load(id) {
            const rows = sql
...
function requireSql(doInstance) {
    ...
    if (!sql) {
        ...
        // next request.
        throw new Error('[poof-flue] DO storage.sql is unavailable. The agent\'s wrangler migration must include ' +
...
function createContextForRequest(id, runId, payload, doInstance, req) {
    ...
    context.init = (async (opts) => {
        const userTools = (opts.tools ?? []);
        let poofTools = [];
        if (opts.poofTools !== false) {
            ...
            try {
                ...
                poofTools = conn.tools;
            }
            catch (err) {
                console.warn('[poof-flue] failed to connect to Poof MCP — continuing without default tools:', err instanceof Error ? err.message : String(err));
            }
            const reserved = new Set(poofTools.map((t) => t.name).filter((n) => typeof n === 'string'));
            for (const t of userTools) {
                if (typeof t.name === 'string' && reserved.has(t.name)) {
                    throw new Error(`[poof-flue] User-declared tool '${t.name}' collides with a Poof platform tool. ` +
...
function createRunStoreForRequest(doInstance) {
    ...
    return createDurableRunStore(requireSql(doInstance));
}
...
function logFlueFiberInterrupt(ctx, agentName) {
    console.warn('[poof-flue] Cloudflare fiber interrupted:', agentName, ctx.name, ctx.snapshot ?? null);
}
...
function parseRunRoute(request) {
    ...
    if (child === 'events')
        return { action: 'events', runId };
    if (child === 'stream')
        return { action: 'stream', runId };
    return null;
}
...
function hydrateProcessEnvFromDoEnv(env) {
...
async function dispatchAgent(request, doInstance, agentName, handler) {
    ...
    return handleAgentRequest({
        ...
        runHandler: (ctx, h) => runWithInstanceContext(doInstance, () => doInstance.keepAliveWhile(() => h(ctx))),
    });
}
...
export function definePoofAgent(name, handler) {
    const cls = class extends Agent {
        async onRequest(request) {
            return dispatchAgent(request, this, name, handler);
        }
        async onFiberRecovered(ctx) {
            ...
            return Agent.prototype.onFiberRecovered.call(this, ctx);
        }
    };
    return cls;
}
...
export function configurePoofAgentsRuntime(webhookAgents) {
...
function resolveAgentBinding(env, name) {
    const className = pascalCase(name);
    const binding = env[className];
    if (!binding || typeof binding.idFromName !== 'function') {
        throw new Error(`[poof-flue] Agent binding ${className} not on env. Check that '${name}' is listed in ` +
            `poof-agents.json and the project has been redeployed.`);
    }
    return binding;
}
function agentRequest(name, sessionId, payload, extraHeaders) {
 ...
 * preserves state.
 */
export async function runAgent(env, name, opts) {
    ...
    if (!res.ok) {
        throw new Error(`[poof-flue] runAgent ${name}/${opts.sessionId} → ${res.status}: ${await res.text().catch(() => '')}`);
    }
 ...
 */
export async function enqueueAgentRun(env, name, opts) {
    const binding = resolveAgentBinding(env, name);
    const stub = binding.get(binding.idFromName(opts.sessionId));
    const res = await stub.fetch(agentRequest(name, opts.sessionId, opts.payload, { 'x-webhook': 'true' }));
    if (!res.ok) {
        throw new Error(`[poof-flue] enqueueAgentRun ${name}/${opts.sessionId} → ${res.status}: ${await res.text().catch(() => '')}`);
    }
    const body = (await res.json());
    if (!body.runId) {
 ...
 */
export async function streamAgent(env, name, opts) {
    const binding = resolveAgentBinding(env, name);
    const stub = binding.get(binding.idFromName(opts.sessionId));
    return stub.fetch(agentRequest(name, opts.sessionId, opts.payload, { accept: 'text/event-stream' }));
}
/**
 ...
 */
export async function getAgentRun(env, name, sessionId, runId) {
    const binding = resolveAgentBinding(env, name);
    const stub = binding.get(binding.idFromName(sessionId));
    const res = await stub.fetch(new Request(`http://agent.internal/agents/${name}/${sessionId}/runs/${runId}`, { method: 'GET' }));
    if (!res.ok) {
        throw new Error(`[poof-flue] getAgentRun ${name}/${sessionId}/${runId} → ${res.status}: ${await res.text().catch(() => '')}`);
    }
 ...
 */
export async function streamAgentRunEvents(env, name, sessionId, runId) {
    const binding = resolveAgentBinding(env, name);
    const stub = binding.get(binding.idFromName(sessionId));
    return stub.fetch(new Request(`http://agent.internal/agents/${name}/${sessionId}/runs/${runId}/stream`, {
        method: 'GET',

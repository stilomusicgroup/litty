 ...
 */
export async function getRpcUrlWithFallback(env) {
    const primary = getPrimaryRpcUrl();
    // Test primary with a lightweight getHealth call
    const healthResp = await rpcFetchWithTimeout(primary, { jsonrpc: '2.0', id: 1, method: 'getHealth' }, RPC_TIMEOUT_MS);
    if (healthResp && healthResp.ok) {
        console.log(`[RPC] Using primary endpoint: ${primary.replace(/\?.*$/, '')}`);
        return primary;
    }
    const helius = getHeliusRpcUrl(env);
    if (helius) {
        const heliusHealth = await rpcFetchWithTimeout(helius, { jsonrpc: '2.0', id: 1, method: 'getHealth' }, RPC_TIMEOUT_MS);
 ...
 */
export async function createConnection(env, commitment = 'confirmed') {

...
function setCachedPrice(price, source) {
 ...
 */
async function tryCoinMarketCap(apiKey) {
    try {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), 5000);
        const res = await fetch('https://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest?symbol=SOL', {
            headers: {
        ...
        console.warn('[PriceCapture] CoinMarketCap fetch failed:', err instanceof Error ? err.message : String(err));
        return null;
    }
}
/**
...
async function tryJupiter() {
    ...
    try {
        ...
        console.warn('[PriceCapture] Jupiter fetch failed:', err instanceof Error ? err.message : String(err));
        return null;
    }
}
/**
 ...
 */
export async function captureSolPriceUsd(cmcApiKey) {
    const cached = getCachedPrice();
    if (cached) {
        console.log(`[PriceCapture] Using cached SOL/USD: $${cached.priceUsd.toFixed(4)} (age=${Date.now() - cached.timestampMs}ms)`);
        return cached;
    }
    ...
    if (cmcApiKey) {
        ...
        if (price !== null) {
            ...
            console.log(`[PriceCapture] CoinMarketCap: $${price.toFixed(4)} at ${new Date(timestampMs).toISOString()}`);
            return { priceUsd: price, timestampMs, source: 'coinmarketcap' };
        }
    }
    else {
        console.warn('[PriceCapture] COINMARKETCAP_API_KEY not provided — skipping primary source');
    }
    // Source 2: Jupiter (fallback)
    const price = await tryJupiter();
    if (price !== null) {
        setCachedPrice(price, 'jupiter');
        console.log(`[PriceCapture] Jupiter fallback: $${price.toFixed(4)} at ${new Date(timestampMs).toISOString()}`);
        return { priceUsd: price, timestampMs, source: 'jupiter' };
    }
    // Both sources failed — signal the caller to retry
    console.error('[PriceCapture] ALL sources failed — cannot provide SOL/USD price for this purchase');
    return null;
}
/**
 ...
 */
export function scaleToMicroUsd(priceUsd) {
    return Math.floor(priceUsd * 1000000);
}
/**
 * Unscale micro-USD back to a USD float.
 * Example: 245_670_000 → 245.67
 */
export function unscaleFromMicroUsd(microUsd) {
 ...
 */
export function calcLamportsFromCapturedPrice(amountUsd, microUsdPrice) {
    const usdPrice = unscaleFromMicroUsd(microUsdPrice);
    return Math.floor((amountUsd / usdPrice) * 1000000000);
}

 ...
 */
import { setSongsBuys, getSongsBuys, getSongs, updateSongs, setSongsSwaps, runGetTokenMintAddressQueryForSongs, runGetTokenBalanceQueryForSongs, } from '../collections/songs.js';
...
export const VAULT_SOL_BUFFER_LAMPORTS = 5000000; // 0.005 SOL fee/rent buffer
// ─── Retry configuration for mainnet durability ──────────────────────────────
const MAX_BONDING_CURVE_RETRIES = 3;
const RETRY_BASE_DELAY_MS = 2000; // 2s base, exponential backoff
const RETRY_MAX_DELAY_MS = 15000; // cap at 15s between retries
/**
...
function isRetryableError(err) {
    ...
    if (lower.includes('timeout') || lower.includes('expired') || lower.includes('timed out'))
        return true;
    // Network congestion / RPC overload
    if (lower.includes('rate limit') || lower.includes('429') || lower.includes('too many requests'))
        return true;
    if (lower.includes('blockhash') || lower.includes('block hash'))
        return true;
    if (lower.includes('recent blockhash') || lower.includes('not found'))
        return true;
    // General retryable patterns
    if (lower.includes('temporarily') || lower.includes('try again') || lower.includes('retry'))
        return true;
    if (lower.includes('transaction was not confirmed') || lower.includes('not confirmed'))
        return true;
    // Helius / RPC-specific
    if (lower.includes('helius') && (lower.includes('error') || lower.includes('failed')))
        return true;
    return false;
}
 ...
 */
export function estimateBondingCurveBuyLamports(args) {
    ...
    const feeLamports = Math.ceil((buyLamports * PUMPFUN_BUY_PLATFORM_FEE_BPS) / 10000);
 ...
 */
export async function ensureVaultHasSongTokens(args) {
    ...
    if (solAmtOverrideLamports !== undefined && solAmtOverrideLamports > 0) {
        ...
        feeLamports = Math.ceil((buyLamports * PUMPFUN_BUY_PLATFORM_FEE_BPS) / 10000);
    ...
    for (let attempt = 0; attempt <= MAX_BONDING_CURVE_RETRIES; attempt++) {
        ...
        if (!buyOk) {
            ...
            return fail('bonding curve buy denied after all retries — likely vault has insufficient SOL ' +
    ...
    try {
        ...
        let storedMint = song?.mintAddress;
 ...
 */
export async function bondingCurveSell(_env, args) {
    const { songId, mintAddress, tokenAmount, walletAddress, slippageBps = PUMPFUN_BUY_SLIPPAGE_BPS, execute = false, } = args;

...
import { validatePoofAuth } from '../lib/poof-auth.js';
import { getPurchases, getManyPurchases, updatePurchases } from '../collections/purchases.js';
import { getSongDetails, getManySongDetails, updateSongDetails } from '../collections/songDetails.js';
import { getSongs, getManySongs, runGetTokenBalanceQueryForSongs, runGetTokenMintAddressQueryForSongs, runGetBondingCurveProgressQueryForSongs, setSongsAirdrops, setSongs, updateSongs } from '../collections/songs.js';
...
import { getEditions, updateEditions, setEditionsMintNFT, setEditionsTransferToken, setEditionsPurchases, getManyEditionsPurchases, } from '../collections/editions.js';
import { setPackPurchases, updatePackPurchases, getPackPurchases, getManyPackPurchases, } from '../collections/packPurchases.js';
import { getPacks } from '../collections/packs.js';
import { PROJECT_VAULT_ADDRESS, LIT_SAVINGS_WALLET, OPERATIONS_WALLET, MAX_DAILY_SOL_SPEND, VAULT_MIN_BALANCE_SOL, SOL_PRICE_MIN_USD, SOL_PRICE_MAX_USD, MIN_PURCHASE_USD, APPLE_PAY_DAILY_CAP_CENTS } from '../constants.js';
import { setArtists, updateArtists, getArtists } from '../collections/artists.js';
import { setOperationsFulfillmentCosts, getManyOperationsFulfillmentCosts, } from '../collections/operationsFulfillmentCosts.js';
import { getManyTransactionAudits } from '../collections/transactionAudits.js';
import { runAllSteps, runStepAirdrop, runStepTreasuryTransfer, runStepInfraFee, runStepFundingFee, SHOPIFY_SHARES, DIRECT_SOL_SHARES } from '../utils/fulfillment-steps.js';
...
import { bondingCurveSell } from '../utils/bonding-curve-buy.js';
 ...
 */
async function sendInsufficientFundsAlert(orderId, estimatedCost, balance) {
 ...
 */
function sanitizeFilterValueOrEmpty(value) {
...
const SOL_PRICE_CACHE_TTL_MS = 60000;
async function fetchSolPriceSafe() {
    const now = Date.now();
...
const TOKEN_PRICE_CACHE_TTL_MS = 30000;
async function fetchPumpFunCoinData(mint) {
    try {
...
async function fetchCoinMarketCapTokenPrice(mint, apiKey) {
    if (!apiKey)
        return null;
    ...
    try {
        ...
        const res = await fetch(`https://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest?address=${mint}&convert=USD`, {
            headers: {
...
async function fetchTokenPriceSafe(mint, cmcApiKey) {
    const now = Date.now();
 ...
 */
async function createShopifyCart(variantId, attributes, env, quantity = 1, note) {
...
const STUDIO_VARIANT_ID = '48722034589924';
// ─── Pack definitions ──────────────────────────────────────────────────
/**
...
async function fulfillPackPurchase(params) {
    ...
    if (!resumePurchaseId) {
        try {
            ...
            if (existingById) {
                const existingStatus = existingById.status;
                ...
                if (!TERMINAL_STATUSES.has(existingStatus)) {
                    console.log(`[fulfillPackPurchase] IDEMPOTENT_RETURN — purchaseId=${purchaseId} already exists ` +
                ...
                console.log(`[fulfillPackPurchase] purchaseId=${purchaseId} exists with terminal status=${existingStatus} — ` +
    ...
    if (!resumePurchaseId) {
        try {
            const existingCompleted = await getManyPackPurchases(`where shopifyOrderId = "${sanitizeFilterValueOrEmpty(orderId)}" and packId = "${sanitizeFilterValueOrEmpty(packId)}" and status = "completed"`);
            if (existingCompleted.length > 0) {
                const prior = existingCompleted[0];
                console.log(`[SAFEGUARD-D] ALREADY_FULFILLED — orderId=${orderId} packId=${packId} prior purchaseId=${prior.id} status=${prior.status}. Returning early to prevent double-spend.`);
                return {
                    purchaseId: prior.id,
    ...
    if (solUsd < solPriceMin || solUsd > solPriceMax) {
        console.error(`[SAFEGUARD-C] PRICE_FEED_SUSPECT — orderId=${orderId} solUsd=${solUsd} (source: ${solPriceSource}) is outside sanity bounds [${solPriceMin}, ${solPriceMax}]`);
    ...
    console.log(`[Preflight] totalRequiredSOL=${estimatedCost.toFixed(6)} SOL — ` +
    ...
    if (!isOffchain) {
        ...
        try {
            ...
            const todayCosts = await getManyOperationsFulfillmentCosts(`where status = "success" and timestamp >= ${todayStart}`);
            const todayTotalSOL = todayCosts.reduce((sum, c) => sum + (c.estimatedCostSOL / 1e9), 0);
            if (todayTotalSOL + estimatedCost > maxDailySpend) {
                console.error(`[SAFEGUARD-A] DAILY_BUDGET_EXCEEDED — orderId=${orderId} ` +
            ...
            console.log(`[SAFEGUARD-A] Daily budget OK — orderId=${orderId} ` +
        ...
        try {
            ...
            if (vaultBalance - estimatedCost < vaultFloor) {
                console.error(`[SAFEGUARD-B] VAULT_FLOOR_BREACH — orderId=${orderId} ` +
            ...
            console.log(`[SAFEGUARD-B] Vault floor OK — orderId=${orderId} ` +
                `vaultBalance=${vaultBalance.toFixed(4)} SOL estimatedCost=${estimatedCost.toFixed(4)} SOL ` +
                `floor=${vaultFloor} SOL projectedBalance=${(vaultBalance - estimatedCost).toFixed(4)} SOL`);
        }
        catch (err) {
    ...
    const mintPreCheck = song?.mintAddress;
        ...
        if (!purchaseCreated) {
            ...
            try {
                ...
                if (existingAfterFail) {
                    ...
                    console.log(`[fulfillPackPurchase] DUPLICATE_POLICY_BLOCK — purchaseId=${purchaseId} already exists ` +
    ...
    console.log(`[Pack Webhook] Purchase ${purchaseId} finalized via runAllSteps: ` +
    ...
    const tokenAmountFinal = finalRecord?.tokenAmount ?? 0;
    const splTxHashFinal = finalRecord?.splTxHash ?? '';
    return {
        purchaseId,
        status: finalStatus,
        errors,
 ...
 */
async function generateClaimToken(purchaseId, email, walletAddress, expiresAt) {
    const secret = process.env.CLAIM_TOKEN_SECRET;
    if (!secret)
        throw new Error('CLAIM_TOKEN_SECRET not configured');
 ...
 */
async function verifyClaimToken(token, purchaseId, email, walletAddress) {
...
function buildOgHtml(params) {
    ...
    const escUrl = (u) => u.replace(/'/g, '&#39;').replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
...
function buildReceiptEmail(params) {
    const { songTitle, artistName, coverImage, editionNumber, tokenAmount, tokenSymbol, splTxHash, nftTxHash, currentPrice, } = params;
 ...
 */
async function verifyShopifyHmac(secret, body, hmacHeader) {
    const encoder = new TextEncoder();
    const key = await crypto.subtle.importKey('raw', encoder.encode(secret), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
...
export function registerRoutes(app) {
    ...
    app.get('/api/songs/:songId/holders', ipRateLimitMiddleware(), async (c) => {
        try {
            ...
            if (!mintAddress)
                return ApiErrors.badRequest(c, 'Song has no mint address');
            ...
            const holderCount = largestAccounts.value.filter((account) => account.uiAmount != null && account.uiAmount > 0).length;
    ...
    app.post('/api/songs/:songId/sell', async (c) => {
        ...
        const result = await bondingCurveSell(c.env, {
            songId,
            mintAddress,
            tokenAmount,
            walletAddress,
    ...
    app.get('/api/purchases/by-order/:orderId', async (c) => {
        ...
        const ownedResults = results.filter((r) => r.buyerAddress === walletAddress);
        // Return only safe fields — never expose buyerEmail or buyerAddress
        const safeResults = ownedResults.map((r) => ({
            id: r.id,
            status: r.status,
            songId: r.songId,
            nftCount: r.nftCount,
    ...
    app.post('/api/purchases/claim/:purchaseId', ipRateLimitMiddleware(10), async (c) => {
        ...
        if (packPurchase && packPurchase.status === 'pending_wallet') {
            ...
            try {
                ...
                if (existingUsers.length === 0) {
                    ...
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Claim] Created user record for ${email} with wallet ${walletAddress}`);
                }
            }
            catch (err) {
            ...
            try {
                if (c.env.LOG_LEVEL !== 'silent')
                    console.log(`[Claim] Running runAllSteps for pack purchase ${purchaseId} (wallet now: ${walletAddress})`);
                const stepsResult = await runAllSteps(purchaseId);
                const claimStatus = stepsResult.finalStatus === 'completed' ? 'completed' : 'partial';
                if (c.env.LOG_LEVEL !== 'silent')
                    console.log(`[Claim] runAllSteps complete for ${purchaseId}: tokens=${stepsResult.stepTokens} payout=${stepsResult.stepPayout} notify=${stepsResult.stepNotify} → ${claimStatus}`);
                return sendSuccess(c, {
                    purchaseId,
                    status: claimStatus,
                    walletAddress,
        ...
        try {
            ...
            if (airdropCreated) {
                if (c.env.LOG_LEVEL !== 'silent')
                    console.log(`[Claim] Airdrop created: ${airdropId} of ${tokenAmount} to ${walletAddress}`);
            }
            else {
        ...
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Claim] Purchase ${purchaseId} claimed by wallet ${walletAddress}`);
        return sendSuccess(c, {
            purchaseId,
            status: 'completed',
            walletAddress,
    ...
    app.post('/api/editions/purchase', ipRateLimitMiddleware(10), async (c) => {
        ...
        const existingBuyerPurchases = await getManyEditionsPurchases(editionId, `where buyerAddress = '${sanitizedBuyerAddress}' and status = 'completed'`);
        ...
        const existingPayIdPurchases = await getManyEditionsPurchases(editionId, `where orderId = '${sanitizedPayId}'`);
        ...
        const remainingAfter = editionAfterDecrement?.remaining ?? -1;
        if (remainingAfter < 0) {
            ...
            if (minTokensRequired <= 0) {
                ...
                if (!hasFullAccess) {
                    try {
                        ...
                        const purchases = await getManyPackPurchases(`where buyerAddress = "${sanitizedWallet}" and songId = "${sanitizedSongId}" and status = "completed"`);
    ...
    app.get('/api/og/song/:songId', ipRateLimitMiddleware(), async (c) => {
        ...
        if (symbol)
            descriptionParts.push(`Ticker: $${symbol}`);
        if (genre)
            descriptionParts.push(genre);
    ...
    app.post('/api/webhooks/packs/fulfill', async (c) => {
        ...
        const FULFILLMENT_FROZEN = c.env.FULFILLMENT_FROZEN === 'true';
        if (FULFILLMENT_FROZEN) {
            if (c.env.LOG_LEVEL !== 'silent')
                console.log('[Pack Webhook] FROZEN — returning 200 without enqueueing (vault drain freeze 2026-05-19)');
        ...
        try {
            ...
            if (existing) {
                if (c.env.LOG_LEVEL !== 'silent')
                    console.log(`[Pack Webhook] Duplicate webhook id=${webhookId} — returning 200 without reprocessing`);
        ...
        const packLineItems = lineItems.filter((item) => PACK_CONFIG.some(p => p.sku === item.sku));
        ...
        const cartAttrs = Array.isArray(payload.attributes)
            ? Object.fromEntries(payload.attributes.map((a) => [a.key ?? a.name, a.value]))
            : {};
        const noteAttrs = Array.isArray(payload.note_attributes)
            ? Object.fromEntries(payload.note_attributes.map((a) => [a.name, a.value]))
            : (payload.note_attributes ?? {});
        const lineProps = Array.isArray(payload.line_items?.[0]?.properties)
            ? Object.fromEntries(payload.line_items[0].properties.map((p) => [p.name, p.value]))
            : {};
        ...
        const purchaseAmountUsd = cartAmountUsdStr
        ...
        if (!songId && packLineItems.length > 0) {
            for (const item of packLineItems) {
                ...
                if (sku.startsWith('song-')) {
                    ...
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Pack Webhook] songId resolved from line_item.sku (song- prefix): ${songId}`);
                    break;
                }
                ...
                if (sku && !PACK_CONFIG.some(p => p.sku === sku)) {
                    songId = sku;
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Pack Webhook] songId resolved from line_item.sku (raw): ${songId}`);
                    break;
                }
            }
        }
            ...
            for (const item of lineItems) {
                ...
                for (const prop of item.properties) {
                    if (songIdKeys.has(prop.name) && prop.value) {
                        songId = prop.value;
                        if (c.env.LOG_LEVEL !== 'silent')
                            console.log(`[Pack Webhook] songId resolved from line_item.properties["${prop.name}"]: ${songId}`);
                        break;
                    }
                }
                if (songId)
        ...
        if (buyerWallet) {
            if (c.env.LOG_LEVEL !== 'silent')
                console.log(`[Pack Webhook] Resolved buyer wallet via ${walletSource}: ${buyerWallet.slice(0, 6)}...${buyerWallet.slice(-4)} (order ${orderId})`);
        }
        if (!buyerWallet) {
        ...
        const purchaseWalletSource = cartAttrs.walletSource ?? noteAttrs.walletSource ?? 'shopify';
        ...
        let adminOrder = adminResult.order;
        if (!adminResult.verified) {
            ...
            switch (adminFailReason) {
                ...
                case 'auth_failed':
                    ...
                    console.warn(`[Pack Webhook] Admin API auth failed for order ${orderId} (401/403) — ` +
            ...
            if (c.env.LOG_LEVEL !== 'silent')
                console.log(`[Pack Webhook] shopify_admin_verified=false reason=${adminFailReason} order=${orderId} — using HMAC-only data`);
        }
        else {
            ...
            if (c.env.LOG_LEVEL !== 'silent')
                console.log(`[Pack Webhook] shopify_admin_verified=true financial_status=${adminOrder.financial_status} order=${orderId}`);
            ...
            if (adminOrder.email) {
                payload.email = adminOrder.email;
            }
            if (Array.isArray(adminOrder.line_items) && adminOrder.line_items.length > 0) {
                payload.line_items = adminOrder.line_items;
            }
            if (Array.isArray(adminOrder.note_attributes)) {
                payload.note_attributes = adminOrder.note_attributes;
            }
            if (adminOrder.total_price !== undefined) {
                payload.total_price = adminOrder.total_price;
            }
        }
        ...
        const verifiedLineItems = Array.isArray(payload.line_items)
            ? payload.line_items
            : lineItems;
        const verifiedNoteAttrs = Array.isArray(payload.note_attributes)
            ? Object.fromEntries(payload.note_attributes.map((a) => [a.name, a.value]))
            : noteAttrs;
        const verifiedTotalPriceStr = String(payload.total_price ?? totalPrice ?? '0');
        ...
        let verifiedBuyerWallet = buyerWallet; // start with pre-verified value
        if (adminOrder) {
            // Admin order available — re-resolve wallet from Admin-sourced note_attributes and line_item.properties.
            const adminNoteAttrs = verifiedNoteAttrs;
            const adminLineProps = Array.isArray(verifiedLineItems[0]?.properties)
                ? Object.fromEntries(verifiedLineItems[0].properties.map((p) => [p.name, p.value]))
                : {};
            ...
            const isValidSolAddr = (addr) => /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(addr);
        ...
        const verifiedPackLineItems = verifiedLineItems.filter((item) => PACK_CONFIG.some(p => p.sku === item.sku));
        ...
        for (const item of finalPackLineItems) {
            ...
            const existingPurchases = await getManyPackPurchases(`where shopifyOrderId = "${sanitizeFilterValueOrEmpty(orderId)}" and packId = "${sanitizeFilterValueOrEmpty(resolvedPackId)}"`);
            if (existingPurchases.length > 0) {
                ...
                if (isPartial && existing.paymentReconciliation) {
                    try {
                        const recon = JSON.parse(existing.paymentReconciliation);
                ...
                if (isCompleted) {
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Pack Webhook] Dedup: order ${orderId} + pack ${resolvedPackId} already fulfilled (status: completed, purchaseId: ${existing.id}), skipping`);
                ...
                if (isPartial && partialHasFailedStep && verifiedBuyerWallet) {
                    ...
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Pack Webhook] Dedup: order ${orderId} + pack ${resolvedPackId} is partial with failed steps (purchaseId: ${existing.id}), re-enqueuing`);
                    try {
                ...
                if (isPartial) {
                    ...
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Pack Webhook] Dedup: order ${orderId} + pack ${resolvedPackId} already partial (purchaseId: ${existing.id}), skipping`);
                    webhookResults.push({ purchaseId: existing.id, status: 'already_completed', errors: [] });
                    continue;
                }
                if (isPendingWalletRecord && verifiedBuyerWallet) {
                    ...
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Pack Webhook] Dedup: order ${orderId} + pack ${resolvedPackId} has pending_wallet record (purchaseId: ${existing.id}), resuming with wallet ${verifiedBuyerWallet}`);
                    try {
                ...
                if (existing.status === 'failed' || existing.status === 'partial_failure') {
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Pack Webhook] Dedup: order ${orderId} + pack ${resolvedPackId} status=${existing.status} (purchaseId: ${existing.id}), re-enqueuing via pack-fulfillment queue`);
                    try {
                        await enqueueQueue(c, 'pack-fulfillment', {
                            purchaseId: existing.id,
                            email: verifiedEmail,
                ...
                if (existing.status === 'pending_price_fetch') {
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Pack Webhook] Dedup: order ${orderId} + pack ${resolvedPackId} status=pending_price_fetch (purchaseId: ${existing.id}), re-enqueuing`);
                    try {
                        await enqueueQueue(c, 'pack-fulfillment', {
                            purchaseId: existing.id,
                            email: verifiedEmail,
                ...
                if (c.env.LOG_LEVEL !== 'silent')
                    console.log(`[Pack Webhook] Dedup: order ${orderId} + pack ${resolvedPackId} exists with status ${existing.status} (purchaseId: ${existing.id}), skipping`);
            ...
            try {
                ...
                if (result.status === 'duplicate_purchase') {
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Pack Webhook] DUPLICATE_POLICY_BLOCK for order ${orderId} pack ${resolvedPackId} ` +
                ...
                if (result.purchaseId && result.status !== 'failed' && !isPendingWallet && !alreadyDone) {
                    ...
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Pack Webhook] Enqueued pack-fulfillment job for purchase ${result.purchaseId} (order ${orderId})`);
                    webhookResults.push({ purchaseId: result.purchaseId, status: 'enqueued', errors: [] });
                }
                else {
                    webhookResults.push({ purchaseId: result.purchaseId, status: result.status, errors: result.errors });
        ...
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Pack Webhook] Webhook processing complete for order ${orderId}: ${webhookResults.length} packs queued`);
        // ── Move Apple Pay reservation to fulfilled ────────────────────────────────
        void (async () => {
            try {
                const orderDateKey = new Date(payload.created_at ?? Date.now()).toISOString().slice(0, 10);
                const budget = await get(`applePayDailyBudget/${orderDateKey}`);
                if (!budget)
                    return;
                const reservations = budget.reservations ? JSON.parse(budget.reservations) : [];
    ...
    app.post('/api/admin/artists/:artistAddress/verify', async (c) => {
        ...
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Admin] Artist ${artistAddress} isVerified set to ${isVerified} by ${walletAddress}`);
    ...
    app.post('/api/admin/payouts/retry/:purchaseId', async (c) => {
        ...
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Admin Payout Retry] Retrying payout for ${purchaseId} by ${walletAddress}`);
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Admin Payout Retry] artistAddress: ${songCreator}, purchaseAmountUsd: ${purchase.purchaseAmountUsd}`);
    ...
    app.post('/api/packs/checkout', async (c) => {
        ...
        let checkoutUrl;
        try {
            const cart = await createShopifyCart(variantId, attributes, c.env);
    ...
    app.post('/api/packs/checkout/open-amount', ipRateLimitMiddleware(10), async (c) => {
        ...
        try {
            const walletLog = resolvedWallet
                ...
                : 'wallet=guest';
            if (c.env.LOG_LEVEL !== 'silent')
                console.log(`[open-amount] Creating Storefront cart: variantId=${OPEN_AMOUNT_VARIANT_ID} qty=${quantity} amountUsd=${priceStr} ${walletLog}`);
            const cart = await createShopifyCart(OPEN_AMOUNT_VARIANT_ID, cartAttributes, c.env, quantity, noteJson);
            checkoutUrl = cart.checkoutUrl;
            cartId = cart.cartId;
            if (c.env.LOG_LEVEL !== 'silent')
                console.log(`[open-amount] Storefront cart created: cartId=${cartId} checkoutUrl=${checkoutUrl}`);
            ...
            try {
                ...
                const existingReservations = freshBudget?.reservations ? JSON.parse(freshBudget.reservations) : [];
    ...
    app.post('/api/admin/purchases/fulfill-manual', async (c) => {
        ...
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Manual Fulfill] Admin ${adminWallet} manually fulfilling purchase ${purchase.id} for buyer ${body.buyerWallet}`);
    ...
    app.post('/api/admin/purchases/fulfill-new', async (c) => {
        ...
        if (!buyerWallet)
            return ApiErrors.badRequest(c, 'buyerWallet is required');
        if (!packSku)
            return ApiErrors.badRequest(c, 'packSku is required');
        // Validate the wallet address format
        try {
        ...
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Fulfill-New] Admin ${adminWallet} creating new pack purchase for order ${orderId}, email ${email}, pack ${packSku}, wallet ${buyerWallet}`);
        ...
        for (const songId of songIds) {
            try {
                ...
                if (existing) {
                    ...
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Sync-to-Chain] Song ${songId} already exists onchain, skipping`);
                    continue;
                }
                ...
                if (created) {
                    ...
                    if (c.env.LOG_LEVEL !== 'silent')
                        console.log(`[Sync-to-Chain] Song ${songId} synced onchain successfully`);
                    ...
                    try {
                        ...
                        if (realMint && realMint.length > 0) {
                            await updateSongs(songId, { mintAddress: realMint });
                            if (c.env.LOG_LEVEL !== 'silent')
                                console.log(`[Sync-to-Chain] Stored mintAddress for song ${songId}: ${realMint}`);
                        }
                        else {
    ...
    app.get('/api/admin/packs/dry-run', async (c) => {
        ...
        try {
            ...
            const tierResults = tiersToTest.map((packConfig) => {
                ...
                const readiness = {
                    packConfig: 'PASS',
                    tokenAmountCalculation: tokenAmount > 0 ? 'PASS' : 'FAIL',
                    artistPayoutCalculation: solUsd > 0 ? 'PASS' : 'WARN_FALLBACK',
                    platformFeeCalculation: solUsd > 0 ? 'PASS' : 'WARN_FALLBACK',
                    balanceCheck: balanceCheckError ? 'FAIL' : (balanceSufficient ? 'PASS' : 'FAIL'),
                    songDetailsAvailable: songDetailsAvailable ? 'PASS' : 'WARN_USE_FALLBACK',
                };
    ...
    app.get('/api/admin/shopify/order/:orderNumber', async (c) => {
        ...
        if (!orderNumber)
            return ApiErrors.badRequest(c, 'orderNumber param is required');
        ...
        try {
            ...
            if (msg.includes('invalid admin token'))
                return ApiErrors.internal(c, 'Shopify Admin API token is invalid or expired');
    ...
    app.get('/api/admin/shopify/recent-orders', async (c) => {
        ...
        try {
            ...
            const purchasesByOrderId = {};
            for (const p of recentPurchases) {
                const oid = String(p.shopifyOrderId ?? '');
                if (!oid)
                    continue;
                if (!purchasesByOrderId[oid])
                    purchasesByOrderId[oid] = [];
    ...
    app.post('/api/admin/purchases/auto-fulfill-by-order', async (c) => {
        ...
        if (!orderNumber)
            return ApiErrors.badRequest(c, 'orderNumber is required');
        ...
        if (!buyerWallet) {
            ...
            const existing = await getManyPackPurchases(`where shopifyOrderId = "${sanitizeFilterValueOrEmpty(orderId)}"`);
            if (existing.length > 0 && existing[0].buyerAddress) {
                buyerWallet = existing[0].buyerAddress;
                if (c.env.LOG_LEVEL !== 'silent')
                    console.log(`[Auto-Fulfill] Found existing purchase wallet: ${buyerWallet}`);
            }
        }
        if (!buyerWallet) {
            ...
            if (users.length > 0 && users[0].walletAddress) {
                ...
                if (c.env.LOG_LEVEL !== 'silent')
                    console.log(`[Auto-Fulfill] Found user wallet by email: ${buyerWallet}`);
            }
        }
        if (!buyerWallet) {
            return ApiErrors.badRequest(c, `Could not determine buyer wallet for order #${orderNumber} (email: ${email}). ` +
        ...
        const noteAttrs = Array.isArray(order.note_attributes)
            ? Object.fromEntries(order.note_attributes.map((a) => [a.name, a.value]))
            : {};
        ...
        if (!songId) {
            ...
            if (songs && songs.length > 0) {
                songId = songs[0].id;
                if (c.env.LOG_LEVEL !== 'silent')
                    console.log(`[Auto-Fulfill] Using first available song: ${songId}`);
            }
        }
        if (!songId) {
        ...
        const existingPurchases = await getManyPackPurchases(`where shopifyOrderId = "${sanitizeFilterValueOrEmpty(orderId)}" and packId = "${sanitizeFilterValueOrEmpty(packConfig.id)}"`);
        ...
        const totalPriceCents = packConfig.priceUsd;
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Auto-Fulfill] Admin ${adminWallet} auto-fulfilling order #${order.order_number} ` +
    ...
    app.post('/api/admin/purchases/fulfill-by-shopify-id', async (c) => {
        ...
        if (!inputOrderId)
            return ApiErrors.badRequest(c, 'shopifyOrderId is required');
        ...
        if (!buyerWallet) {
            ...
            if (existing.length > 0 && existing[0].buyerAddress) {
                buyerWallet = existing[0].buyerAddress;
                if (c.env.LOG_LEVEL !== 'silent')
                    console.log(`[Fulfill-By-ID] Found existing purchase wallet: ${buyerWallet}`);
            }
        }
        if (!buyerWallet) {
            const users = await getManyUsers(`where email = "${sanitizeFilterValueOrEmpty(email)}"`);
            if (users.length > 0 && users[0].walletAddress) {
                buyerWallet = users[0].walletAddress;
                if (c.env.LOG_LEVEL !== 'silent')
                    console.log(`[Fulfill-By-ID] Found user wallet by email: ${buyerWallet}`);
            }
        }
        if (!buyerWallet) {
            return ApiErrors.badRequest(c, `Could not determine buyer wallet for order ${inputOrderId} (email: ${email}). ` +
                'Pass buyerWallet in the request body to override.');
        }
        try {
            new PublicKey(buyerWallet);
        ...
        if (!songId) {
            ...
            if (songs && songs.length > 0) {
                songId = songs[0].id;
                if (c.env.LOG_LEVEL !== 'silent')
                    console.log(`[Fulfill-By-ID] Using first available song: ${songId}`);
            }
        }
        if (!songId) {
            return ApiErrors.badRequest(c, 'Could not determine songId. Pass songId in the request body to override.');
        ...
        const totalPriceCents = packConfig.priceUsd;
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Fulfill-By-ID] Admin ${adminWallet} fulfilling order id ${orderId} (#${order.order_number}), ` +
    ...
    app.post('/api/admin/purchases/repair/:purchaseId', async (c) => {
        ...
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Repair Purchase] admin=${adminWallet} purchaseId=${purchaseId}`);
        ...
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Repair Purchase] Enqueued purchase ${purchaseId} as job ${job.jobId}`);
        return sendSuccess(c, {
            purchaseId,
            jobId: job.jobId,
            status: 'enqueued',
    ...
    app.post('/api/admin/purchases/:id/resume', async (c) => {
        ...
        if (!existing)
            return ApiErrors.notFound(c, `No packPurchases record found for ${id}`);
        const allowedStatuses = ['failed', 'partial_failure', 'partial', 'pending_price_fetch', 'pending'];
        if (!allowedStatuses.includes(existing.status)) {
            return ApiErrors.badRequest(c, `Purchase ${id} has status "${existing.status}" — only ${allowedStatuses.join(', ')} records can be resumed`);
        }
        try {
    ...
    app.post('/api/admin/purchases/:id/reenqueue', async (c) => {
        ...
        const purchase = await getPackPurchases(purchaseId);
        if (!purchase)
            return ApiErrors.notFound(c, `Purchase ${purchaseId} not found`);
        const allowedStatuses = ['failed', 'partial_failure', 'partial', 'pending_price_fetch', 'pending'];
    ...
    app.post('/api/admin/purchases/:id/cancel', async (c) => {
        ...
        const id = c.req.param('id');
        if (!id)
            return ApiErrors.badRequest(c, 'Purchase ID is required');
        const { get: dbGet, set: dbSet } = await import('@pooflabs/server');
        const existing = await dbGet('packPurchases/' + id);
        if (!existing)
            return ApiErrors.notFound(c, 'Purchase record not found');
    ...
    app.post('/api/direct-sol-purchase', async (c) => {
        ...
        if (!txSignature)
            return ApiErrors.badRequest(c, 'txSignature is required');
        if (!songId)
            return ApiErrors.badRequest(c, 'songId is required');
        if (!usdAmount || usdAmount < 1)
            return ApiErrors.badRequest(c, 'usdAmount must be >= 1');
        if (!buyerAddress)
            return ApiErrors.badRequest(c, 'buyerAddress is required');
        ...
        let verifiedLamports = 0;
        {
            try {
                ...
                const opsIndex = accountKeys.findIndex((k) => k.toBase58?.() === opsAddress);
                if (opsIndex < 0) {
                ...
                return ApiErrors.badRequest(c, `On-chain verification failed: ${err?.message ?? 'RPC error'}. Please retry in a moment.`);
            }
        }
        ...
        // for SOL price drift between the time the buyer submitted the tx and now.
        {
            ...
            if (usdAmount > actualUsd * (1 + TOLERANCE)) {
                return ApiErrors.badRequest(c, `Claimed amount ($${usdAmount}) exceeds on-chain payment (~$${actualUsd.toFixed(2)} at current SOL price). Please retry with the correct amount.`);
            }
        }
        ...
        const song = await getSongs(songId);
        if (!song)
            return ApiErrors.notFound(c, `Song ${songId} not found`);
        ...
        const packConfig = PACK_CONFIG.reduce((closest, p) => Math.abs(p.priceUsd - approxPriceUsdCents) < Math.abs(closest.priceUsd - approxPriceUsdCents) ? p : closest, PACK_CONFIG[0]);
    ...
    app.get('/api/admin/pending-orders', async (c) => {
        const { walletAddress } = await validatePoofAuth(c, true);
        const pendingOrders = await getManyPackPurchases("where status = 'pending_wallet' or status = 'pending_oversold' or status = 'partial' order by createdAt asc limit 100");
        const result = (pendingOrders ?? []).map((p) => ({
            orderId: p.id,
    ...
    app.post('/api/admin/fulfill', async (c) => {
        ...
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Admin Fulfill] Starting fulfillment for orderId=${orderId}, buyerWallet=${buyerWallet || 'not provided'}`);
        ...
        try {
            ...
            const { _emailMeta: _ignored, ...resultWithoutMeta } = result;
            if (result.status === 'completed') {
                if (c.env.LOG_LEVEL !== 'silent')
                    console.log(`[Admin Fulfill] SUCCESS for orderId=${orderId}: purchaseId=${result.purchaseId}`);
    ...
    app.get('/api/admin/pending-songs', async (c) => {
        ...
        const pending = allDetails.filter(d => d.approvalStatus === 'pending' ||
    ...
    app.post('/api/admin/songs/:songId/approve', async (c) => {
        ...
        if (launchMode === 'auto') {
            ...
            try {
                const vaultClient = await createWalletClient({ keypair: c.env.TAROBASE_SOLANA_KEYPAIR });
        ...
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Admin Approve] mintAddress confirmed after ${mintAttempts} attempt(s): ${realMintAddress}`);
        // Write mint address to database
        const mintWritten = await updateSongs(songId, { mintAddress: realMintAddress });
        if (!mintWritten) {
        ...
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[Admin Approve] Captured and stored mintAddress for song ${songId}: ${realMintAddress}`);
        // ─────────────────────────────────────────────────────────────────────────
        ...
        if (!ok)
            return ApiErrors.internal(c, 'Failed to update song details');
    ...
    app.post('/api/admin/songs/:songId/set-paused', async (c) => {
        ...
        if (!song)
            return ApiErrors.notFound(c, 'Song not found');
        const ok = await updateSongs(songId, { paused: body.paused });
        if (!ok)
            return ApiErrors.internal(c, 'Failed to update song');
    ...
    app.post('/api/admin/songs/:songId/set-hidden', async (c) => {
        ...
        if (!song)
            return ApiErrors.notFound(c, 'Song not found');
        const ok = await updateSongs(songId, { hidden: body.hidden });
        if (!ok)
            return ApiErrors.internal(c, 'Failed to update song');
    ...
    app.post('/api/admin/songs/:songId/set-swap-eligible', async (c) => {
        ...
        if (!song)
            return ApiErrors.notFound(c, 'Song not found');
        const ok = await updateSongs(songId, { swapEligible: body.swapEligible });
        if (!ok)
            return ApiErrors.internal(c, 'Failed to update song');
    ...
    app.post('/api/songs/:songId/stream', async (c) => {
        ...
        const dedupRecord = await get(`streamEvents/${dedupId}`);
        if (dedupRecord) {
    ...
    app.post('/api/admin/retry-fulfillment', async (c) => {
        ...
        if (!fulfillmentId)
            return ApiErrors.badRequest(c, 'fulfillmentId is required');
        const record = await getFailedFulfillments(fulfillmentId);
        if (!record)
            return ApiErrors.notFound(c, `Failed fulfillment record ${fulfillmentId} not found`);
    ...
    app.delete('/api/admin/failed-fulfillments/:fulfillmentId', async (c) => {
        ...
        const record = await getFailedFulfillments(fulfillmentId);
        if (!record)
            return ApiErrors.notFound(c, 'Failed fulfillment record not found');
        const deleted = await deleteFailedFulfillments(fulfillmentId);
        if (!deleted)
            return ApiErrors.internal(c, 'Failed to delete record — check admin policy');
    ...
    app.post('/api/admin/purchases/:purchaseId/recover-trapped-tokens', async (c) => {
        ...
        const parsed = schema.safeParse(body);
        if (!parsed.success)
            return ApiErrors.badRequest(c, `Invalid body: ${parsed.error.issues.map(i => i.message).join(', ')}`);
        ...
        if (!buyerAddress)
            return ApiErrors.badRequest(c, 'Purchase has no buyerAddress — wallet must be set before recovery');
        ...
        try {
            ...
            const transferOk = await set(`recoveryTransfers/${recoveryId}`, {
                purchaseId,
                buyerAddress: Address.publicKey(buyerAddress),
            ...
            if (c.env.LOG_LEVEL !== 'silent')
                console.log(`[recover-trapped-tokens] recoveryTransfers hook succeeded: recoveryId=${recoveryId} txHash=${txSig} — ${amountStr} tokens of ${mintAddress} → ${buyerAddress}`);
        }
        catch (err) {
        ...
        if (c.env.LOG_LEVEL !== 'silent')
            console.log(`[recover-trapped-tokens] Recovery complete for ${purchaseId}: recoveryId=${recoveryId} txSig=${txSig} amount=${humanAmount} (${amountStr} base units) → ${buyerAddress}`);
        return sendSuccess(c, {
            success: true,
            recoveryId,
            txSig,
    ...
    function buildSyntheticCandles(snapshots, bucketSeconds) {
    ...
    app.get('/api/songs/:mint/candles', ipRateLimitMiddleware(), async (c) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                candlesCache.set(cacheKey, { data: payload, expiresAt: now + 30000 });
    ...
    app.patch('/api/support/ticket/:id', async (c) => {
        ...
        if (!ticketId)
            return ApiErrors.badRequest(c, 'ticket id is required');
        let body;
        try {
            body = await c.req.json();
        }
    ...
    app.post('/api/error-log', async (c) => {
        ...
        const env = c.env?.ENV ?? c.env?.WORKER_ENV ?? 'unknown';
    ...
    app.patch('/api/error-log/:id/resolve', async (c) => {
        ...
        if (!logId)
            return ApiErrors.badRequest(c, 'log id is required');
